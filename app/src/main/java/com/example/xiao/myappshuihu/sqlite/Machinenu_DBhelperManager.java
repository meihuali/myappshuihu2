...
public class Machinenu_DBhelperManager {
	...
	public synchronized Machine getMachine(String id) {
		...
		try {
			db = open();
			cursor = db.query(DATABASE_TABLE,null,"M_ID=?",new String[]{id},null,null,null);

			if(cursor.moveToNext()){
				data.machineid=cursor.getString(cursor.getColumnIndex(M_ID));
				data.name=cursor.getString(cursor.getColumnIndex(MNAME));
				return data;
			}
		} catch (Exception e) {
			// TODO: handle exceptionn
