 ...
 * 描述：数据库操作类 曾删改查
 */
public class MyGreenDao_settings_Utils {
    /*这个是保存数据*/
    public static SettingsDataSqlBeanDao saveData(Context context) {
        DaoMaster.DevOpenHelper devOpenHelper = new DaoMaster.DevOpenHelper(context, "mydbsss", null);
        DaoMaster daoMaster = new DaoMaster(devOpenHelper.getWritableDatabase());
        DaoSession daoSession = daoMaster.newSession();
        SettingsDataSqlBeanDao userDao = daoSession.getSettingsDataSqlBeanDao();
        return userDao;
    }
     ...
     * 查询用户列表 也可以说是取数据
     */
    public static List<SettingsDataSqlBean> getqueryUserList(Context context) {
        DaoMaster.DevOpenHelper devOpenHelper = new DaoMaster.DevOpenHelper(context, "mydbsss", null);
        DaoMaster daoMaster = new DaoMaster(devOpenHelper.getWritableDatabase());
        DaoSession daoSession = daoMaster.newSession();
        SettingsDataSqlBeanDao userDao = daoSession.getSettingsDataSqlBeanDao();
        List<SettingsDataSqlBean> userList = userDao.queryBuilder()
                .where(SettingsDataSqlBeanDao.Properties.Id.notEq(0))
