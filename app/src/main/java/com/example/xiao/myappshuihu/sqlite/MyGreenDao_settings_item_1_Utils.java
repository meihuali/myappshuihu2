 ...
 * 描述：数据库操作类 曾删改查
 */
public class MyGreenDao_settings_item_1_Utils {
    /*这个是保存数据*/
    public static SettingsDataSql_item1_BeanDao saveData(Context context) {
        DaoMaster.DevOpenHelper devOpenHelper = new DaoMaster.DevOpenHelper(context, "mydbssss", null);
        DaoMaster daoMaster = new DaoMaster(devOpenHelper.getWritableDatabase());
        DaoSession daoSession = daoMaster.newSession();
        SettingsDataSql_item1_BeanDao userDao = daoSession.getSettingsDataSql_item1_BeanDao();
        return userDao;
    }
     ...
     * 查询用户列表 也可以说是取数据
     */
    public static List<SettingsDataSql_item1_Bean> getqueryUserList(Context context) {
        DaoMaster.DevOpenHelper devOpenHelper = new DaoMaster.DevOpenHelper(context, "mydbssss", null);
        DaoMaster daoMaster = new DaoMaster(devOpenHelper.getWritableDatabase());
        DaoSession daoSession = daoMaster.newSession();
        SettingsDataSql_item1_BeanDao userDao = daoSession.getSettingsDataSql_item1_BeanDao();
        List<SettingsDataSql_item1_Bean> userList = userDao.queryBuilder()
                .where(SettingsDataSql_item1_BeanDao.Properties.Id.notEq(0))
