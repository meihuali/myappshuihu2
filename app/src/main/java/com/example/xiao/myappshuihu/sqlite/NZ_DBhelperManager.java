...
public class NZ_DBhelperManager {
	...
	private static final String DATABASE_CREATE = "create table "
			...
			+ "ZDY_ID text  not null, " + "MACHINE_ID text  not null, " + "ZDY_NAME text  not null, "
	...
	public synchronized List<NZYData> getAlermList(String machineid) {
		List<NZYData> list = new ArrayList<NZYData>();
	...
	public synchronized boolean delete(NZYData data) {
		...
		initialValues.put(NZ_START, data.NZ_START);
		// initialValues.put(ORDER_ID, "order_id");
		boolean bol = db.delete(DATABASE_TABLE, "NZ_ID=?",new String[]{ data.NZ_ID}) > 0;
		db.close();
		return true;
	}

	public Cursor getAll(String machineid) {
		Cursor cur = db.rawQuery("SELECT * FROM nz_quickdial where MACHINE_ID = ? ",new String[]{machineid});
		return cur;
	}
	...
	public void addDefault(List<DBhelperManager.ZDYData> mList, String machineid){
		...
		for (int i=0; i<mList.size()-1;i++ ) {
			...
			data.NZ_MS = mList.get(i).ZDY_NAME;
			data.MACHINE_ID=machineid;
			data.NZ_TIME=timeString[i];
			try {
				insert(data);
